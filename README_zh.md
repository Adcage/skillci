...
[![Tests](https://img.shields.io/badge/tests-167%20passed-brightgreen.svg)](#测试)
...
| `skillci test <path> --mode llm` | 运行 LLM 触发测试 |
| `skillci test <path> --mode both` | 同时运行本地和 LLM |
| `skillci test <path> --json` | 输出 JSON 报告 |
| `skillci test <path> --no-cache` | 禁用 LLM 缓存 |
| `skillci test <path> --compare latest` | 与基线快照对比 |
| `skillci snapshot <path>` | 保存基线快照 |
| `skillci report <path> --format markdown` | 生成 Markdown 报告 |
| `skillci report <path> --format github` | 生成 PR 友好报告 |
...
| `judge.cache` | bool | `true` | 启用 LLM 结果缓存 |
